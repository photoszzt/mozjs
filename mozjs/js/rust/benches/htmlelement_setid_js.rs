...
#[bench]
fn bench_htmlelement_setid_js(_b: &mut Bencher) {
    ...
    unsafe {
        ...
        rooted!(in(cx) let mut script1 = ptr::null_mut() as *mut JSScript);
...
"#,
                                  "test1", 22, script1.handle_mut());
        JS_ExecuteScript(cx, script1.handle(), rval.handle_mut());
    }
}
