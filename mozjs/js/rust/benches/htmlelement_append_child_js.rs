...
#[bench]
fn bench_htmlelement_append_child_js(_b: &mut Bencher) {
...
function bench(num) {
    ...
    for ( var i = 0; i < 2000; i++) {
        let element1 = new HtmlElement(1, "Node" + i, "mozilla/en", false, "n", "h1", [], "la", "a", "l", "pp",
        "foo", [], "title123", "en", false, "dir12345", false, 1, "ackeylab", "ackeylab", false, false);
        elems.push(element1);
    }
    var ret;
    ...
    for ( var i = 0; i < num; i++) {
        for ( var j = 0; j < 2000; j++) {
            node.appendChild(elems[j]);
        }
    }
    duration = (gettime() - t1) / 1000000.0;
}
bench(1024);
puts("");
puts("appendChild time is");
