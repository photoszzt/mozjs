...
#[bench]
fn bench_htmlelement_setid_normaljs(_b: &mut Bencher) {
    ...
    unsafe {
        ...
        rooted!(in(cx) let mut script3 = ptr::null_mut() as *mut JSScript);
...
bench(102400);
puts("");
puts("normal js setid time is");
puts(val_to_str(duration));
"#,
                                  "test3", 14, script3.handle_mut());
        JS_ExecuteScript(cx, script3.handle(), rval.handle_mut());
    }
}
