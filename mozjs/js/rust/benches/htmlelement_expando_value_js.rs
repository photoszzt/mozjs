...
function bench(num) {
    ...
    let attr3 = new Attr(1, "Node3", "mozilla/en", false, "n", "h1", "lc", "c", "l", "p", "f");
    let attr4 = new Attr(1, "Node4", "mozilla/en", false, "n", "h1", "ld", "d", "l", "p", "b");
    ...
    var t1 = Date.now();
    for ( var i = 0; i < num; i++) {
        element1["test" + num] = function(){};
    }
    duration = Date.now() - t1;
    var t2 = Date.now();
    var ret;
    for ( var i = 0; i < num; i++) {
        ret = element1["test" + num];
    }
    duration2 = Date.now() - t2;
}
bench(102400);
puts("");
puts("element.expando = value time is");
puts(val_to_str(duration));
puts("element.expando time is");
puts(val_to_str(duration2));
"#,
